...
<body>
	...
	<div class="row">
		...
		<?php 
		// recent login data recive using cookie id start
		if (isset($_COOKIE['rememLogin'])) {
			$rlogin_id = $_COOKIE['rememLogin'];

			$sql = "SELECT * FROM user_info WHERE id = '$rlogin_id'";
				$data = $connection -> query($sql);
				$login_user = $data -> fetch_assoc();

		}
		//recent login data recive using cookie id End
		if (isset($_COOKIE['rememLogin'])): ?>

			<div class="car shadow w-25 mx-auto" style="margin-top:7%; margin-left: 2%">
				<a href="?triger=<?php echo $login_user['id'];?>">
					<div class="card-body w-25">
					<img style="max-height: 100px;" src="photos/<?php echo $login_user['photo'];?>" alt="">
					</div>
				</a>
				
				<div class="card-footer">
					<a href="?task=clear">Clear</a> <?php echo $login_user['uname'];?>
				</div>
			</div>
		<?php endif ?>
		...
		<?php
		if (isset($_GET['task']) AND ($_GET['task'] = 'clear') ) {
			setcookie('rememLogin','',time() - (60*60*24*365));
